...
/**
 * This class represents the KMeans algorithm in all its iterative steps Extends
 * SwingWorker in order to be executed as a background Thread and to keep
 * control of progress.
 */
public class KMeans extends SwingWorker<Void, Void> {
	...
	/**
	 * Method used to complete step 1 of the Kmeans algorithm. It initalizes the
	 * various centroids
	 */
	...
	private Centroid generateCentroid(int currentpos) {
		...
		return new Centroid((((x1 - x2) / (clusters.size() + 1)) * currentpos)
				+ x1, (((y1 - y2) / (clusters.size() + 1)) * currentpos) + y1);
	}
	 ...
	 * the user closes the operation. Note: The setProgress() method calls are
	 * used to keep the ProgressBar up to date
	 * 
	 * 1. Initialize centroid 2. assign Instances randomly to clusters 3.
	 * Perform the initial evaluation of distances --> Done directly when adding
	 * a Instance to a cluster 4. Refresh Centroids at least once 5. Start the
	 * main computation and stop if we no more moves can be done or the selected
			...
			for (int j = 0; j < clusters.size(); j++) {
				for (int k = 0; k < clusters.get(j).getNumInstances(); k++) {
					if (isCancelled())
						return null;
					// Refresh progress
					progress = Math.max((Math.round((float) ((i + 1) * (j + 1)
							* (k + 1) * 100)
							/ (n)) + 1), progress);
					setProgress(Math.min(progress - 1, 100));
					double currentED = clusters.get(j).getInstance(k)
							.getCurrentEuDt();
					Cluster temp = null;
					boolean toreassign = false;
					// Look if cluster has to be reassigned
					for (int l = 0; l < clusters.size(); l++) {
						if (currentED > clusters
								.get(j)
								.getInstance(k)
					...
					if (toreassign) {
						...
						clusters.get(j).removeInstance(
								clusters.get(j).getInstance(k));
						refreshCentroids();
					}
				}
				if (clearnoise) {
					...
					compactness = clusters.get(j).getNumInstances()
							/ (clusters.get(j).getNumOfMinAndMaxXY());
					// Calculate cluster compactness =number of data points /
					// compactness window size
	...
	/**
	 * Method is used for step 2. It randomly assigns various Instances to
	 * Cluster
	 */
